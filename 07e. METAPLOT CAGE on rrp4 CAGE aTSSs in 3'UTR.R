...
setwd('~/Dropbox/Axel_Arabidopsis_Flagellin/ANALYSIS_TSSstory/07 - metagene plots antisense')

# CAGE SCALED METAPLOT ON RNASEQ RRP4 ADEGS (FROM DEEPTOOLS) ####
# ---------------------------------------------------------------
pp <- read.delim('~/Dropbox/Axel_Arabidopsis_Flagellin/ANALYSIS_TSSstory/07 - metagene plots antisense/matrix_CAGE_plus_BED_plus.mat', skip=2)[, -1]
mm <- read.delim('~/Dropbox/Axel_Arabidopsis_Flagellin/ANALYSIS_TSSstory/07 - metagene plots antisense/matrix_CAGE_minus_BED_minus.mat', skip=2)[, -1]
pm <- read.delim('~/Dropbox/Axel_Arabidopsis_Flagellin/ANALYSIS_TSSstory/07 - metagene plots antisense/matrix_CAGE_plus_BED_minus.mat', skip=2)[, -1]
mp <- read.delim('~/Dropbox/Axel_Arabidopsis_Flagellin/ANALYSIS_TSSstory/07 - metagene plots antisense/matrix_CAGE_minus_BED_plus.mat', skip=2)[, -1]
...
#mat_sense_med %<>% mutate('position'=1:240, 'direction'='sense')
#mat_anti_med %<>% mutate('position'=1:240, 'direction'='anti')
...
# gg_med <- rbind(mat_sense_med, mat_anti_med) %>%
#   melt(id.vars=c('position', 'direction'), variable.name='genotype', value.name='averageRPM') %>%
#   subset(direction=='anti') %>%
#   ggplot(aes(x=position, y=ifelse(direction=='anti', -averageRPM, averageRPM), group=interaction(direction, genotype), col=genotype)) +
#          geom_vline(xintercept=c(20, 220), lty=2) +
#          geom_line(lwd=.65) +
#          cowplot::theme_cowplot() + theme(aspect.ratio=1, legend.position='bottom', legend.direction=2) +
#          labs(title='CAGE at rrp4 CAGE 3UTR aTSSs (N=176)',
#               x='', y='Median CAGE TPM') +
#          scale_color_brewer(palette='Set2', name='', direction=-1)